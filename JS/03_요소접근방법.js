...
function accessClass(){
    ...
    console.log(arr);
    // [div.area.div2, div.area.div2, div.area.div2]로 출력됨. (배열 O)
...
function acessTagName(){
    ...
    console.log(arr);
    // [li, li, li, li, li] 로 배열형태를 가짐.

    for(let i = 0 ; i < arr.length; i++){
        const num = arr[i].innerText;
        
        arr[i].style.backgroundColor = "rgb(250,220," + (50*num) + ")";
    }
}
...
function accessCss(){
    ...
    // 아이디는 # , 클래스는 .을 이용하여 어떤 선택자를 가져왔는지 명시해야 함.

    document.querySelector("#css-div > div").style.fontSize = "30px";
    //  여러개가 있는 경우 첫번재 요소만 변경함

    // querySelectorAll : 모든 요소 변경하기
...
function readValue(){
    ...
    if(input.value.trim().length > 0 ){
        ...
        // 스크롤의 높이를 scrollTop에 대입함.
        // 요소.scrollTop  : 요소 내부 현재 스크롤 위치 반환 (맨 위 = 0 )
        // 요소.scrollHeight : 스크롤의 전체 높이
        // 요소.scrollTop = 요소.scrollHeight :스크롤을 현재 스크롤 높이만큼 위치를 이동
